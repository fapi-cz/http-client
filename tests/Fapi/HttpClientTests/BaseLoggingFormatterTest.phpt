...
final class BaseLoggingFormatterTest extends TestCase
{
	...
	public function testFormatFailed(): void
	{
		...
		Assert::equal(
			'Fapi\HttpClient: an HTTP request failed. Request URL: "test.cz" Request method: "GET" Request headers: [] '
			. 'Request body: "" Exception type: "Exception" Exception message: "test" Elapsed time: 100.14 ms',
			$formatted,
		);
	}
	...
	public function testFormatSuccessful(): void
	{
		...
		Assert::equal(
			'Fapi\HttpClient: an HTTP request has been sent. Request URL: "test.cz" Request method: "GET" '
			. 'Request headers: [] Request body: "" Response status code: 200 Response headers: [] '
			. 'Response body: "{\"test\": \"test\"}" Elapsed time: 101.14 ms',
			$formatted,
		);
	}
